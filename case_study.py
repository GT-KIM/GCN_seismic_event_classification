...
rootpath = "/sdc/Eq2020_multisite_0925/Case_study/case_study_multi_noise/"
...
class MyDataset(Dataset) :
    ...
    def __getitem__(self, idx) :
        ...
        data_YOCB = np.concatenate((self.data_YOCB[0].data[:,np.newaxis],
                               self.data_YOCB[1].data[:,np.newaxis],
                               self.data_YOCB[2].data[:,np.newaxis]), axis=1)
        data_USN2 = np.concatenate((self.data_USN2[0].data[:,np.newaxis],
                               self.data_USN2[1].data[:,np.newaxis],
                               self.data_USN2[2].data[:,np.newaxis]), axis=1)
        data = np.concatenate((data_PHA2[...,np.newaxis],
                               data_YOCB[...,np.newaxis],
                               data_USN2[...,np.newaxis]), axis=2)
...
test_all_2 = data_YOCB[2].data
test_all_2 = test_all_2 - np.mean(test_all_2)
test_all_3 = data_USN2[2].data
test_all_3 = test_all_3 - np.mean(test_all_3)
result_list = np.array(result_list)
x = np.arange(len(result_list)) / len(result_list) * 12
fig, axes = plt.subplots(4, 1, sharex=True, sharey=True, figsize=(18, 18))
plt.title("[{} {} {}]".format(result[0], result[1], result[2]))
axes[0,0].plot(x, test_all_1)
axes[0,1].plot(x, test_all_2)
axes[0,2].plot(x, test_all_3)
axes[0,3].plot(x, result_list)
plt.savefig("/sdc/Eq2020_multisite_0925/case_study_event.jpg")
plt.show()